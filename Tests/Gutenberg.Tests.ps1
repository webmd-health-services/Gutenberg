...
Describe ('Gutenberg.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_Gutenberg' -Exists
    }
}

Describe ('Gutenberg.command verbs') {
...
Describe ('Gutenberg.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Gutenberg' -CommandType Function,Cmdlet,Filter))
        {
